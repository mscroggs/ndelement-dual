...
use ndgrid::traits::{Topology, Grid, Entity};
...
>(
        ...
        .entity_iter(ReferenceCellType::Interval)
    {
        ...
        for (vi, coarse_v_index) in edge.topology().sub_entity_iter(ReferenceCellType::Point).enumerate() {
            let fine_v_index = refined_grid.fine_vertex(ReferenceCellType::Point, coarse_v_index);
            let fine_v = fine_grid.entity(ReferenceCellType::Point, fine_v_index).unwrap();

            let mut fine_faces = fine_v.topology()
                .connected_entity_iter(ReferenceCellType::Triangle).collect::<Vec<_>>();

            let fine_edges = fine_v.topology()
                .connected_entity_iter(ReferenceCellType::Interval).collect::<Vec<_>>();

            let mut ordered_edges = vec![];
            let mut next_edge = *fine_edges.iter().filter(|i| {
                let vs = fine_grid.entity(ReferenceCellType::Interval, **i).unwrap().topology().sub_entity_iter(ReferenceCellType::Point).collect::<Vec<_>>();
            ...
            while !fine_faces.is_empty() {
                ...
                for (i, face_index) in fine_faces.iter().enumerate() {
                    let face = fine_grid.entity(ReferenceCellType::Triangle, *face_index).unwrap();
                    let edges = face.topology().sub_entity_iter(ReferenceCellType::Interval).collect::<Vec<_>>();
            ...
            let sign = T::from(match vi {0 => 1, 1 => -1, _ => { panic!("Edge with more than two vertices"); }}).unwrap();
            let total = ordered_edges.len();
            let mut n = total / 2 - 1;

            for e in ordered_edges.iter().skip(1) {
                if n != 0 {
                    let v = fine_grid.entity(ReferenceCellType::Interval, *e).unwrap().topology()
                        .sub_entity_iter(ReferenceCellType::Point).filter(|i| *i != fine_v_index)
                        .next().unwrap();
                    let edge_sign = if v > fine_v_index { sign } else { -sign };
                    let edofs = fine_space.entity_dofs(ReferenceCellType::Interval, *e).unwrap();
                    c.insert(edofs[0], edge_sign * T::from(n).unwrap() / T::from(total).unwrap());
                    n -= 1;
                }
            }

            for e in first_face.unwrap().topology().sub_entity_iter(ReferenceCellType::Interval) {
                let vs = fine_grid.entity(ReferenceCellType::Interval, e).unwrap().topology()
                    .sub_entity_iter(ReferenceCellType::Point).collect::<Vec<_>>();
            ...
            for e in last_face.unwrap().topology().sub_entity_iter(ReferenceCellType::Interval) {
                let vs = fine_grid.entity(ReferenceCellType::Interval, e).unwrap().topology()
                    .sub_entity_iter(ReferenceCellType::Point).collect::<Vec<_>>();
                if !vs.contains(&fine_v_index) {
                    let v = if vs[0] == edge_point { vs[1] } else { vs[0] };
